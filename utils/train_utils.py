...
#     args.experiment = "-".join([args.experiment, args.norm])
    if not args.resume_training:
        args.experiment = "-".join([args.experiment, datetime.now().strftime("%b-%d-%H:%M:%S")])

    dataloader = args.dataloader
#     if args.data == 'cifar10C':
#         args.severity_list.sort()
#         dataset += ''.join([str(x) for x in args.severity_list])
        
    args.experiment_dir = os.path.join(args.output_dir, dataloader, args.model, (f"drafts/" if args.draft else "") + args.experiment)
