...
class Probing_model(pl.LightningModule):
    ...
    def __init__(self,layer,from_scratch = True,mse_btwin = False, ckpt_path = None):
        ...
        self.final_fc = nn.Linear(fc_dim,14)
...
def main(args):
    ...
    if args.mse_btwin == 'moco-mse':
        ckpt_path = args.ckpt_path
    elif args.mse_btwin == 'moco-btwin':
        ckpt_path = args.ckpt_path
    else:
        ckpt_path = args.ckpt_path
        
    prob_model = Probing_model(args.probe_layer,args.from_scratch,args.mse_btwin, ckpt_path)
    filename = 'probing-new-'
    if args.mse_btwin == 'moco-mse' or args.mse_btwin == 'moco-btwin':
        filename += args.mse_btwin
    else:
        filename += 'mocov2only'
    filename += '-'+args.probe_layer+'-'+str(args.from_scratch)+'-{epoch:02d}-{val_auc_score:.3f}'
...
def get_args():
    ...
    #Add Probe arguments
    parser.add_argument("--probe_layer", type=str)
    parser.add_argument('--from_scratch', type=str2bool, nargs='?',const=True, default=False)
    parser.add_argument("--mse_btwin", default=None, type=str, help = "one of 'moco', 'moco-mse', or 'moco-btwin'")
    parser.add_argument("--ckpt_path", type=str, help = 'Path to ckpt (pretrained ssl) for which we need to do the probing')
